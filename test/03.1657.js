...
var closeStrings = function (word1, word2) {
  if (word1.length !== word2.length) return false;
  const arrayCount01 = new Array(26).fill(0), arrayCount02 = new Array(26).fill(0);
  for (let i = 0; i < word1.length; i++) {
    arrayCount01[word1.charCodeAt(i) - 97]++;
    arrayCount02[word2.charCodeAt(i) - 97]++;
  }
  for (let i = 0; i < arrayCount01.length; i++) {
    if (
      (arrayCount01[i] !== 0 && arrayCount02[i] === 0)
      || (arrayCount02[i] !== 0 && arrayCount01[i] === 0)
    ) {
      return false;
    }
  }
  arrayCount01.sort();
  arrayCount02.sort();
  return arrayCount01.toString() === arrayCount02.toString();
}
